class Game {
  constructor() {
    ...
    //Activar en caso de colision
    this.leftKeyActive = false;

    //DECLARAR BANDERA DE COLISION 
    this.blast = false;
  }
  ...
  start() {
    ...
    car1.scale = 0.07;
     ...
     //CARGAR LA IMAGEN DE LA COLISION 
  ...
  play() {
    ...
    if (allPlayers !== undefined) {
      ...
      for (var plr in allPlayers) {
        ...
        //DECLARAR UNA VARIABLE LOCAL DONDE ESTE LA VIDA DE LOS JUGADORES
        var currentlife = allPlayers[plr].life;
       //----CONSTRUIR CONDICIONAL PARA PROYECTAR IMAGEN DE COLISION
        if(currentlife <= 0){
        ...
        if (index === player.index) {
          ...
          //LLAMAR A LA FUNCION COLISION CARRO VS CARRO
          this.handleCarACollisionWithCarB(index);
          
          //CREAR CONDICIONAL PARA CAMBIAR LAS BANDERAS DE MOVIMIENTO Y IMG DE COLICION
          if (player.life <=0){
            this.blast = true;
            this.playerMoving = false;
  ...
  handlePlayerControls() {
    //incluyo la bandera para al chocar inmovilizar al carro
    if (!this.blast){
...
//------generamos la funcion que tectecta la colision entre los carros
